...
export const bulkDeleteDeals = wrapServiceFn(
  async function bulkDeleteDeals(ids, tenantId, userContext) {
...
export const bulkDeleteLeads = wrapServiceFn(
  async function bulkDeleteLeads(ids, tenantId, userContext) {
    if (!Array.isArray(ids) || ids.length === 0) {
      throw new AppError("ids array is required and must not be empty", 400);
    }
...
export const bulkDeleteOrganizations = wrapServiceFn(
  async function bulkDeleteOrganizations(ids, tenantId, userContext) {
    if (!Array.isArray(ids) || ids.length === 0) {
      throw new AppError("ids array is required and must not be empty", 400);
    }
